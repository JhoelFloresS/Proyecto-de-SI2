...
*/
console.log('BPMNScript.ts');

import { DrawCommandHandler } from 'gojs/extensionsTS/DrawCommandHandler.ts';
import 'gojs/extensionsTS/Figures.ts';
import * as go from 'gojs/release/go.js';
import { BPMNLinkingTool, BPMNRelinkingTool, PoolLink } from 'gojs/projects/bpmn/BPMNClasses.ts';
  ...
  if (checkLocalStorage()) {
    ...
    if (saveName === UnsavedFileName) {
      ...
      for (const key in window.localStorage) {
        ...
        if( key === saveName){
          id_diagram = words[0];
          name_diagram = words[1];
          break
        }
      }

      //petition put with fetch
      fetch(`/api/${tenant}/diagramas/${id_diagram}/update`, {
        method: 'PUT',
        headers: {
...
export function saveDocumentAs() {
  if (checkLocalStorage()) {
    ...
    if (saveName && saveName !== UnsavedFileName) {
      ...
      saveDiagramProperties();
      window.localStorage.setItem(saveName, myDiagram.model.toJson());
      //petition post with fetch
      fetch(`/api/${tenant}/diagramas/store`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
...
export function openDocument() {
  if (checkLocalStorage()) {
    if (myDiagram.isModified) {
      const save = confirm('Le gustaria guardar los cambios en' + getCurrentFileName() + '?');
      if (save) {
        saveDocument();
      }
    }
...
export function loadFile() {
  ...
  for (let i = 0; i < listbox.options.length; i++) {
    if (listbox.options[i].selected) fileName = listbox.options[i].value; // selected file
  }
...
export function removeFile() {
  ...
  if (fileName !== undefined) {
    ...
    // if the file that was deleted was the current file, then the current file is set to be the default file
    if (fileName === getCurrentFileName()) {
      setCurrentFileName(UnsavedFileName);
      myDiagram.model = new go.GraphLinksModel();
    }
    //petition delete with fetch

    
    fetch(`/api/${tenant}/diagramas/${id_diagram}/delete`, {
      method: 'DELETE',
      headers: {
        'Content-Type': 'application/json'
      }
...
export function updateFileList(id: string) {
  ...
  for (const key in window.localStorage) {
    ...
    option.value = key;
    //get the words after - of key
    const words = key.split('-');
    option.text = words[words.length - 1];
